  ...
  return (
    <div className="group flex flex-col overflow-hidden rounded-3xl border border-ink/[0.07] bg-white transition-all duration-300 hover:-translate-y-1.5 hover:shadow-card dark:border-cream/10 dark:bg-ink-800">
      <Link to={`/product/${product.id}`} className="relative block aspect-square overflow-hidden bg-cream-100 dark:bg-white/5">
        ...
        {off > 0 && <span className="absolute right-2.5 top-2.5 chip bg-lime-400 text-ink nums">{off}% OFF</span>}
        {!product.inStock && (
      ...
      <div className="flex flex-1 flex-col p-3.5">
        ...
        <div className="mt-auto flex items-end justify-between pt-3">
          <div>
            <div className="font-display text-lg font-semibold text-ink dark:text-cream nums">{inr(product.price)}</div>
            {off > 0 && <div className="text-xs text-ink/40 dark:text-cream/40 line-through nums">{inr(product.mrp)}</div>}
          </div>
